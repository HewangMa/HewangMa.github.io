...
void test09() {
	...
	cout << "at least evacuting! \n----------------------\n\n";

	BinTreeNode<int>* p1 = new BinTreeNode<int>(4);
	BinaryTree<int> tr(p1);
	int i = 0;
	BinTreeNode<int>* p = tr.getRoot();
	while (i < 10) {
		BinTreeNode<int>* p2 = new BinTreeNode<int>(i++);
		if (tr.Insert(p, p2, 1)) cout << "insert of "<<i<<" is done!\n";
		p = p2;
	}

	cout << "tr.height: " << tr.Height() << endl;

	cout << "\n\n----------------------\ngood job! all done! Now tring to destroy! \n\n";
}
class Sulotion {
public:
	vector<vector<int>> threeSum(vector<int>& nums) {
		...
		for (;i <= n - 3;i++) {
			for (j = i + 1;j <= n - 2;j++) {
				if (nums[i] + nums[j] > 0)goto breakLoop;
				for (k = j + 1;k <= n - 1;k++) {
	...
	vector<string> letterCombinations(string digits) {
		...
		int i = 0;
		map<int, string> m;
		m.emplace(make_pair(2, "abc"));
		m.emplace(make_pair(3, "def"));
		m.emplace(make_pair(4, "ghi"));
		m.emplace(make_pair(5, "jkl"));
		m.emplace(make_pair(6, "mno"));
		m.emplace(make_pair(7, "pqrs"));
		m.emplace(make_pair(8, "tuv"));
		m.emplace(make_pair(9, "wxyz"));
		for (int j = 0;j < m[i].size();j++)	ans.emplace_back(m[i][j]);
		while (i < n) {
	...
	int mySqrt(int x) {
		...
		while (1) {
			unsigned long long k = mid * mid, k1 = (mid + 1) * (mid + 1);
			cout << "mid: "<<mid<<"\tk: " << k << "\tk1 : " << k1 << endl;
			if (k <= x && k1 > x) break;
			if (k > x) {
				right = mid;
				mid = rand() % (right - left) + left;
				continue;
			}
			if (k1 <= x) {
				left = mid;
	...
	void sortColors(vector<int>& nums) {
		...
		while (i < j) {
			...
			while (nums[i] != 0) i++;
			while (nums[j] != 2)j--;
			swap(nums[i], nums[left]);left = i;i++;
			swap(nums[j], nums[right]);right = j;j++;
		}
	}
};
void test10() {
	...
	//int a; cin >> a;
	//cout << So.mySqrt(a) << endl;
	//cout << pow(2, 31) - 1 << endl;

	//int b = cin.get();cout << "b: "<<b << endl;

	vector<int> s = { 0,1,2,1,1,2,2,1,2,1,0,1,2,2,1,2,1,2 };
	So.sortColors(s);
	copy(s.begin(), s.end(), ostream_iterator<int>(cout, " "));cout << endl;


	clock_t t2 = clock();
	cout <<"the running time: "<<(double)(t2 - t1) / CLOCKS_PER_SEC << endl;
	//cout << endl;
}
class ele {
public:
	static int st;
	ele(int i, int j) :a(i), b(j) {};
	int a;
	int b;
	friend ostream& operator<<(ostream& out, const ele& a);
	friend void dun(const ele& a) {
		cout << a.a << endl;
	}
};
...
ostream& operator<<(ostream& out, const ele& a) {
	out << "a: " << a.a << "\tb:" << a.b;
	return out;
}
void test11() {
	ele a(1, 5), b(234, 247), c(67, 9),d(2537,65);
	vector<ele> s{ a,b,c,d };
	sort(s.begin(), s.end(), comp);
	copy(s.begin(), s.end(), ostream_iterator<ele>(cout, "\n"));
}
void test12() {
	...
	vector<int>::iterator newEnd=unique(it_1, it_2);
	cout << "去重后的 a : ";
